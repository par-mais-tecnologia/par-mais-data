...
class ParMaisData {
  ...
  identify (eventName, id, userProperties = undefined, eventData = undefined) {
    return this.core().pipz.identify(eventName, id, userProperties, eventData)
  }

  event (name, data = undefined) {
    return this.core().pipz.event(name, data)
  }

  persisData (data) {
    return window.fetch(this.targetUrl, {
      method: 'POST',
      mode: 'cors',
      headers: {
  ...
  getData (id) {
    return window.fetch(`${this.targetUrl}${id}/`, {
      method: 'GET',
      mode: 'cors',
      headers: {
        'accept-encoding': 'gzip, deflate',
